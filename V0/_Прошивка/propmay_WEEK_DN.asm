...
;******************************************************************************
	...
	de  "ÂÒÎÐÍÈÊ",0x00
    de  "*ÑÐÅÄÀ*",0x00
    de  "×ÅÒÂÅÐÃ",0x83,0x84,0x00
    de  0x83,0x84,"ÏßÒÍÈÖÀ",0x81,0x82,0x00
    de  0x81,0x82,"ÑÓÁÁÎÒÀ",0x81,0x82,0x00
    de  0x81,0x82,"ÂÎÑÊÐÅÑÅÍÜÅ",0x81,0x82,0x00
    de  0x83,0x84, "ÊÒÎ ÂÅÐÈÒ Â ÑÂÎÞ ÓÄÀ×Ó - !",0x81,0x82,0x81,0x82,0x81,0x82,0x00
;**********************************************************************
...
lHour
		...
		incf	Week,f		; week = week + 1
        incf   	Day,f		; Day = Day + 1
lWeek
		movlw	.8		; 
		subwf	Week,w		; week < 8 ?
		btfss	STATUS,C        ; if week < 8 then c=1 (borrow)
		goto	lDay		; 
					; yes, week >= 7
		movwf	Week		; week = week - 7
		incf	Week,f
lDay		
		movlw	.32		; DMon = 32
		movwf	DMon		
...
cldisp_2
		...
		bsf	STATUS,RP0	; goto bank 1
         ...
         btfsc	fDemo
         call startstart		; inc address for next read
	     movwf	EEADR
...
startstart  movlw	.7		; 
		 subwf	Weeked,w		; week < 7 ?
		 btfss	STATUS,Z    ;if week < 7 then c=0(borrow)
		 goto week1
         movlw	0x3E
         return
       
week1    movlw	.6		; 
		 subwf	Weeked,w		; week < 8 ?
		 btfss	STATUS,Z
         goto week2
