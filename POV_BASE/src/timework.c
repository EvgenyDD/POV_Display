...
/*******************************************************************************
* Function Name  : TimeDateToStruct
* Description    : Convert stm32f051 time struct and date struct to stm32f103 timeDate struct
*******************************************************************************/
void TimeDateToStruct(RTC_TimeTypeDef *TimeTemp, RTC_DateTypeDef *DateTemp, RTC_TimeDateTypeDef *RTCmass)
{
	RTCmass->second = TimeTemp->RTC_Seconds;
	RTCmass->minute = TimeTemp->RTC_Minutes;
	RTCmass->hour = TimeTemp->RTC_Hours;

	RTCmass->date = DateTemp->RTC_Date;
	RTCmass->month = DateTemp->RTC_Month;
	RTCmass->year = DateTemp->RTC_Year;
}


/*******************************************************************************
* Function Name  : TimeDateToCounter
* Description    : Convert stm32f051 time struct and date struct to stm32f103 counter value
*******************************************************************************/
uint32_t TimeDateToCounter(RTC_TimeTypeDef *TimeTemp, RTC_DateTypeDef *DateTemp)
{
	RTC_TimeDateTypeDef RTC_TD_Temp;

	TimeDateToStruct(TimeTemp, DateTemp, &RTC_TD_Temp);
	return RTC_TimeDateToCnt(&RTC_TD_Temp);
}


/*******************************************************************************
* Function Name  : GetTImeDate
* Description    : Get stm32f051 Time and Date
*******************************************************************************/
void GetTimeDate(RTC_TimeTypeDef *TimeTemp, RTC_DateTypeDef *DateTemp)
{
	RTC_GetTime(RTC_Format_BIN, TimeTemp);
	RTC_GetDate(RTC_Format_BIN, DateTemp);
}
